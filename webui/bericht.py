...
def _virtualisierung() -> str:
    ...
    **"none" ist eine Antwort und keine Fehlanzeige** -- sie heisst: Der
    Server laeuft auf Blech. Bis zum 05.09.2026 stand das Wort ungefiltert
    auf der Karte, weil der Rueckfall nur bei leerer Ausgabe griff. Auf
    einer VM faellt das nie auf; genau auf den Maschinen, um die es geht
    -- den bootenden Blechkisten --, stand dann "none".
    """
...
# --------------------------------------------------------------------------
# Dieselben Angaben als Karte
# --------------------------------------------------------------------------
#
# **Die Karte auf Server Health und der Bericht sagen dasselbe, und das ist
# der Grund, warum sie hier zusammen stehen.** Wer einen Fehler meldet,
# schickt den Bericht; wer ihn liest, fragt nach dem, was in der Karte
# steht. Zwei Quellen liefen frueher oder spaeter auseinander, und dann
# stuende in der Meldung ein anderer Kernel als auf dem Schirm.
#
# **Gepuffert, und zwar aus einem anderen Grund als der Bericht.** Der
# entsteht auf Klick und darf sich Zeit nehmen. Die Karte entsteht bei
# JEDEM Aufbau von Server Health -- der Seite, die man offen stehen laesst.
# Ein dpkg-query, ein systemd-detect-virt und ein uptime bei jedem Aufruf
# waeren drei Prozessstarts fuer Angaben, die sich zwischen zwei
# Aktualisierungen nie aendern.
#
# Eine Minute, nicht zehn Sekunden wie bei den Diensten: Ein Dienst faellt
# aus, waehrend man hinsieht -- ein Kernel wechselt beim Neustart, und der
# nimmt die Seite ohnehin mit.
KARTEN_TAKT = 60

_KARTE: dict = {"zeit": 0.0, "werte": {}}
...
def karte() -> dict:
    ...
    fuenf mit ihrer Paketversion. Getrennt, weil die zweite Haelfte die
    Frage beantwortet, die eine fremde Fehlermeldung aufwirft:
    **dnsmasq 2.89 und 2.90 verhalten sich beim proxyDHCP unterschiedlich.**

    **Was hier NICHT hineingehoert, ist eine Paketliste.** Die Oberflaeche
    hat keine Anmeldung; jede Zeile mehr ist fuer jeden im Netz eine Zeile
    mehr Einkaufsliste. Aufgenommen wird, was der Bericht braucht -- und
    sonst nichts.
    """
    jetzt = time.monotonic()
    if not _KARTE["werte"] or jetzt - _KARTE["zeit"] > KARTEN_TAKT:
        stand = versionsstand.kurz()
        versionen = dict(_dienstversionen())
        _KARTE["werte"] = {
            "maschine": [
                ("Distribution", _distribution() or "unbekannt"),
                ("Kernel", platform.release() or "unbekannt"),
            ...
            # In der Reihenfolge, in der die Dienste auch sonst stehen --
            # dieselbe Liste wie in der Karte darueber. pxeweb ist dabei
            # kein Paket, sondern diese Anwendung: Ihre Version ist der
            # Stand, den install.sh gestempelt hat.
            "dienste": [
                (name, versionen.get(name)
                 or (stand or "kein Stempel (nicht über install.sh installiert)"))
                for name in dienste.EINHEITEN
            ],
            "takt": KARTEN_TAKT,
        }
        _KARTE["zeit"] = jetzt
