...
with TestClient(pxeapp.app) as c:
    ...
    # Der uebliche Ablauf: ein neues System erst an einem Rechner erproben.
    ...
    # Ein Fund aus dem Betrieb: Ein entpacktes Abbild legt neben casper/
    # noch pool/ und dists/. Die Regel "ein Nachbarordner haelt den
    # Aufstieg auf" hielt den Zaun schon bei casper/ an -- der Eintrag
    # bekam einen Bruchteil seiner Belegung, der Rest stand unter
    # "Sonstiges". Bei einem Upload ist gar
    ...
    #
    # Im September 2026 an einem Kali-netinst auf dev-marlei aufgefallen:
    # Der stand auf "kein Netzwerkstart moeglich", und seine Karte blieb
    # dauerhaft aufgeklappt. Ein Zustand, der bleibt, ist keine Meldung
    # mehr -- er ist eine Eigenschaft, und eine Liste, in der die Haelfte
    # offen steht, hat keine geschlossene mehr.
    ...
    #
    # Im September 2026 auf der veroeffentlichten Seite gefunden:
    ...
    # -- Serverdetails: worauf dieser Server laeuft (A-020)
    #
    # Bis zum 05.09.2026 stand das nur per SSH zur Verfuegung. Der Satz,
    # an dem der Wert haengt: dnsmasq 2.89 und 2.90 verhalten sich beim
    # proxyDHCP unterschiedlich -- ohne die Version beginnt jede fremde
    # Fehlermeldung mit einer Rueckfrage.
    print("\n-- Serverdetails: worauf dieser Server laeuft")
    import bericht as _b

    karte = c.get("/").text.split('id="serverdetails"')[1].split("</section>")[0]
    check("die Karte steht auf Server Health", 'id="serverdetails"' in c.get("/").text)
    for feld in ("Distribution", "Kernel", "Architektur", "Virtualisierung",
                 "Laufzeit"):
        check("... sie nennt %s" % feld, feld in karte)
    check("... und alle fuenf Dienste mit Version",
          all(name in karte for name in _d.EINHEITEN))

    # Die Karte nennt ihren Takt -- wie die Dienste-Karte. Ohne die Zeile
    # liest jemand einen Stand von vorhin als "jetzt".
    check("... sie nennt ihren Takt",
          'class="kartenfuss"' in karte and str(_b.KARTEN_TAKT) in karte)

    # Die Oberflaeche hat keine Anmeldung (B-037). Eine Karte mit allen
    # Paketen waere fuer jeden im Netz eine Einkaufsliste -- aufgenommen
    # ist, was der Bericht braucht, und sonst nichts.
    stand = _b.karte()
    check("die Dienstliste ist genau die der Karte darueber",
          [n for n, _ in stand["dienste"]] == list(_d.EINHEITEN))
    check("... und die Maschine bleibt eine Handvoll Zeilen",
          len(stand["maschine"]) <= 8, "%d Zeilen" % len(stand["maschine"]))

    # Dieselbe Quelle wie der Fehlerbericht: Zwei liefen auseinander, und
    # dann stuende in einer Meldung ein anderer Kernel als auf dem Schirm.
    bericht_text = _b.text(assets, mit_umgebung=False)
    for name, wert in stand["maschine"]:
        if name in ("Distribution", "Kernel", "Architektur"):
            check("%s steht auch im Bericht" % name, wert in bericht_text,
                  "Karte: %r" % wert)

    # Die Hilfe im Netz zeigt keine Karte, die einen laufenden Server
    # meldet -- sonst steht der Kernel dieses Servers eines Tages
    # oeffentlich unter exmig.github.io.
    vorlage = (Path(__file__).resolve().parent.parent
               / "webui" / "templates" / "hilfe.html").read_text(encoding="utf-8")
    check("die Hilfe rendert die Angaben nicht, sie beschreibt sie nur",
          "serverdetails." not in vorlage
          and "serverhealth-serverdetails" in vorlage)

    # "none" ist eine Antwort, keine Fehlanzeige -- und "oracle" heisst
    # VirtualBox. Beides am 05.09.2026 beim Ansehen der fertigen Karte
    # gefunden: Auf einer VM faellt das nie auf, auf Blech stuende "none".
    echt_lauf = _b._lauf
    try:
        _b._lauf = lambda befehl: "none"
        check("keine Virtualisierung sagt, dass der Server auf Blech laeuft",
              "Blech" in _b._virtualisierung())
        _b._lauf = lambda befehl: "oracle"
        check("... und oracle bekommt seinen Klarnamen",
              _b._virtualisierung() == "VirtualBox (oracle)")
        _b._lauf = lambda befehl: ""
        check("... eine leere Antwort bleibt nicht feststellbar",
              _b._virtualisierung() == "nicht feststellbar")
        _b._lauf = lambda befehl: "irgendwas-neues"
        check("... ein unbekannter Wert steht unveraendert da",
              _b._virtualisierung() == "irgendwas-neues")
    finally:
        _b._lauf = echt_lauf
    ...
    # Die zweite Falle, eine Ebene tiefer als die von ufw: "is-active"
    # antwortet "inactive" auch fuer eine Einheit, die es auf dieser
    # Maschine gar nicht gibt. Am 05.09.2026 stand deshalb auf
    # dev-marlei "firewalld -- installiert, aber aus" neben etwas, das
    # nie installiert war. Gefragt wird seither nach LoadState.
    class _Antwort:
        stdout = """Id=nftables.service
LoadState=loaded
...
"""
    ...
    echt = fw.subprocess.run
    fw.subprocess.run = lambda *a, **k: _Antwort()
    try:
        ueber_systemd = fw._systemd(("nftables", "firewalld"))
    finally:
        fw.subprocess.run = echt
    check("eine nicht installierte Einheit wird nicht gemeldet",
          ueber_systemd == [{"name": "nftables", "an": False}],
          "gefunden: %r" % (ueber_systemd,))
